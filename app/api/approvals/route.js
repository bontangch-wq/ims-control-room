...
export async function PATCH(req){
 ...
 const sql=db();const actorPrior=(await sql`select count(*)::int n from approval_decisions d join approvals a on a.id=d.approval_id where a.record_id=${b.record_id} and d.approver_id=${access.profile.id} and d.decision='Approved'`)[0]?.n||0;if(actorPrior>0)return Response.json({error:'Segregation of duties: the same person cannot approve more than one level in the same approval cycle'},{status:409});const pending=(await sql`select a.id,a.workflow_id,a.level_no,l.required_role,l.required_approvals,l.level_name from approvals a join approval_levels l on l.workflow_id=a.workflow_id and l.level_no=a.level_no where a.record_id=${b.record_id} and a.decision='Pending' order by a.level_no limit 1`)[0]; if(!pending)return Response.json({error:'No pending approval found'},{status:404})
