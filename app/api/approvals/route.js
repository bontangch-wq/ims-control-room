...
export async function PATCH(req){
 ...
 if(b.decision==='Rejected'){
  ...
  if(count>=pending.required_approvals){
   ...
   if(next){const eligibleNext=(await sql`select count(*)::int n from app_users where active=true and app_role=${next.required_role}`)[0].n,adminOverrideNext=(await sql`select count(*)::int n from app_users where active=true and app_role in ('Super Admin','IMS Admin') and app_role<>${next.required_role}`)[0].n,availableNext=eligibleNext+adminOverrideNext;if(availableNext<next.required_approvals){await sql`insert into audit_log(actor_id,action,entity_type,entity_id,detail) values(${access.profile.id},'APPROVAL_BLOCKED_CAPACITY','ims_record',${String(b.record_id)},${JSON.stringify({completed_level:pending.level_no,next_level:next.level_no,required_approvals:next.required_approvals,available_approvers:availableNext})}::jsonb)`;return Response.json({error:`Next approval level cannot start: level ${next.level_no} requires ${next.required_approvals} distinct approver(s), but only ${availableNext} eligible approver(s) are available`},{status:409})}await sql`insert into approvals(record_id,workflow_id,level_no,decision) values(${b.record_id},${pending.workflow_id},${next.level_no},'Pending')`;const targets=await sql`select id from app_users where active=true and (app_role=${next.required_role} or app_role in ('Super Admin','IMS Admin'))`;for(const u of targets)await sql`insert into notifications(user_id,record_id,type,message) values(${u.id},${b.record_id},'APPROVAL_REQUIRED',${`${record.record_no} requires ${next.required_approvals} approval(s) at level ${next.level_no}`})`;await notifyOwner(sql,b.record_id,record.owner_id,'APPROVAL_PROGRESS',`${record.record_no} telah menyelesaikan level ${pending.level_no} (${pending.level_name}) dan masuk ke level ${next.level_no}.`,access.profile.id)}
   else{
