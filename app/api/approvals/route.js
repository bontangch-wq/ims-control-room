...
export async function POST(req){
 ...
 const eligible=(await sql`select count(*)::int n from app_users where active=true and app_role=${first.required_role}`)[0].n;if(!eligible&&!['Super Admin','IMS Admin'].includes(first.required_role))return Response.json({error:`Workflow cannot start: no active ${first.required_role} approver is configured`},{status:409})
