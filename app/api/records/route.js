...
export async function GET() {
 ...
 const sql=db(); const rows=await sql`select r.id,r.record_no,r.title,r.module,r.status,r.severity,r.due_date,r.description,coalesce(u.full_name,'Unassigned') owner from ims_records r left join app_users u on u.id=r.owner_id order by r.created_at desc limit 200`; return Response.json(rows)
}
