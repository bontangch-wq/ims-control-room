...
export async function GET(){
 const access=await requireAccess(['IMS Admin']); if(!access.ok) return access.response
...
export async function PATCH(req){
 ...
 const allowed=['IMS Admin','Auditor','Function Owner','Viewer']; if(!allowed.includes(b.app_role)) return Response.json({error:'invalid role'},{status:400})
 const sql=db(); const r=await sql`update app_users set app_role=${b.app_role},active=${Boolean(b.active)} where id=${b.id} returning id,email,full_name,function_name,app_role,active`
 if(!r.length) return Response.json({error:'user not found'},{status:404})
