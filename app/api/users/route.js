...
import { requireAccess, ACCESS, ROLES } from '@/lib/auth/access'
...
export async function POST(req){
 ...
 if(role===ROLES.SUPER_ADMIN&&access.profile.app_role!==ROLES.SUPER_ADMIN) return Response.json({error:'Only Super Admin can provision Super Admin access'},{status:403})
...
export async function PATCH(req){
 ...
 if(access.profile.app_role!==ROLES.SUPER_ADMIN && (target.app_role===ROLES.SUPER_ADMIN||nextRole===ROLES.SUPER_ADMIN)) return Response.json({error:'Only Super Admin can manage Super Admin access'},{status:403})
 ...
 if(target.app_role===ROLES.SUPER_ADMIN&&(nextRole!==ROLES.SUPER_ADMIN||nextActive===false)){const admins=(await sql`select count(*)::int n from app_users where app_role=${ROLES.SUPER_ADMIN} and active=true`)[0]?.n||0;if(admins<=1)return Response.json({error:'At least one active Super Admin must remain'},{status:409})}
