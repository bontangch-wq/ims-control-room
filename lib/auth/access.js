...
export async function currentAccess() {
  ...
  const sql = db()
  let rows = await sql`select id,email,full_name,function_name,app_role,active,auth_user_id from app_users where auth_user_id=${session.user.id} limit 1`
  if (!rows.length && session.user.email) {
    rows = await sql`update app_users set auth_user_id=${session.user.id} where lower(email)=lower(${session.user.email}) and auth_user_id is null returning id,email,full_name,function_name,app_role,active,auth_user_id`
  }
